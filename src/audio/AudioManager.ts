...
const SOUNDS: Record<SoundName, (a: Player) => void> = {
  ...
  'alchemist-fire': (a) => {
    a.noise(0.18, 0.18, 3200, 0, 'highpass');
    a.tone({ freq: 320, freqEnd: 140, duration: 0.14, volume: 0.2, type: 'sine' });
  },
  explosion: (a) => {
  ...
  'shield-break': (a) => {
    a.noise(0.22, 0.3, 6000, 0, 'highpass');
    a.tone({ freq: 1800, freqEnd: 500, duration: 0.2, volume: 0.16, type: 'triangle' });
    a.tone({ freq: 900, freqEnd: 300, duration: 0.24, volume: 0.12, type: 'sine', delay: 0.03 });
  },
  'boss-spawn': (a) => {
